  ...
  */
  ...
  getKeys : function(bucket, cb){
    ...
    this.db.collection(this.prefix + collName,function(err,collection){
      if(err instanceof Error) return cb(err);
      // Excluding bucket field from search result
      collection.find(searchParams, {_bucketname: 0}, function(err, docs){
        if(err) return cb(err);
        if(! _.isObject(docs) ) return cb(undefined,[]);
        var result = [];
        docs.each(function (err, doc){
          if(err) return cb(err);
          if(! _.isObject(doc) ) return cb(undefined,result);
          result.push(doc.key);
        });
      });
    });
