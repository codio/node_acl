...
var Acl = function (backend, logger){
  ...
  backend.getKeysAsync = bluebird.promisify(backend.getKeys);
};
...
/**
 getUsers( resource, function(err, obj) )

 Returns all the allowable users for resource.

 It returns an array of userIds.

 @param {String} resource id.
 @param {Function} Callback called when finished.
 */
Acl.prototype.getUsers = function(resource, cb){
  contract(arguments)
    .params('string')
    .end();

  var collection = allowsBucket(resource);

  return this.backend.getKeysAsync(collection).then(function(allows){
    return allows;
  }).nodeify(cb);
};
